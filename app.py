...
# On Render free tier the repo root is read-only; /tmp is always writable.
# If you attach a Render Disk, set DATABASE_URL env var to e.g. /data/cloud.db
DB_PATH     = os.environ.get("DATABASE_URL", os.path.join("/tmp", "cloud.db"))

app = Flask(__name__, static_folder=STATIC_DIR, static_url_path="")

# Allow same-origin in prod; allow all in dev (FLASK_ENV=development)
...
def login():
    ...
    if user:
        u = dict(user)
        u.pop("password", None)          # never send hash to client
...
def allocate():
    ...
    # Build cost matrix: lower = better fit
    ...
    for i, t in enumerate(pending_tasks):
        for j, v in enumerate(idle_vms):
            ...
            priority_weight  = (6 - t["priority"]) * 10   # higher priority → lower cost
            noise            = random.uniform(0.1, 2.0)   # simulate real-world variance
            cost[i][j]       = cpu_penalty + ram_penalty + priority_weight + noise
    ...
    with get_db() as db:
        for r, c in zip(row_ind, col_ind):
            if r < n_t and c < n_v:
                ...
                alloc_cost    = round(float(cost[r][c]), 4)
